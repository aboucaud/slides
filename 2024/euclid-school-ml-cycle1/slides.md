...
#### Ecole d'été Rodolphe Clédassou 2024 – Cycle 1
...
2. Detail the structure of .red[neural networks] and how to train them

3. Show the limits of classic NN and justify the need for probabilistic networks

4. Practice with a ML framework (.blue[TensorFlow]) on a regression task
...
## Why data driven ?
  ...
  .medium[Physical model .green[too complex] or dataset .green[too large],]  
  .medium[leading to convergence issues] 
...
---
## Soon the first Euclid data release\\0/
...
.medium[There might be .green[hidden information] in the data,]  
.medium[beyond the summary statistics we traditionally compute]
  
.right.big.red[discovery]
...
.medium[specialized .blue[hardware]] 

.medium.right[e.g. GPU, TPU, Intel Xeon Phi]

--
.medium[.blue[data] availability] 

.medium.right[switch to data driven algorithms]

--
.medium[ML .blue[algorithm] research] 

.medium.right[e.g. self-supervised, active learning, ...]

--
.medium[.blue[open source] tools] 

.medium.right[huge ecosystem available in a few clicks]
...
.medium[$$\text{MSE} = \frac{1}{N}\sum_i\left(y_i - f_w(x_i)\right)^2$$]
...
.medium.center[$f_w(x) = y$  ↔️️  $p_w(y|x)$]
...
.medium.center[$$p_\vec{w}(y|x) = L(\vec{w}, \sigma) \propto \prod_i^N \exp(-(y_i - f_w(x_i))^2/\sigma^2)$$]
...
.medium[$$-\log p_\vec{w}(y|x) \propto \sum_i^N (y_i - f_w(x_i))^2/\sigma^2 + cst$$]
...
The same principle can be applied to model increasingly complex distributions
.center[
  <img src="../img/mdn-multi.png" width="75%">
]

---
## A wider goal of machine learning in physics

Learn the inner data distribution $X$ of your system with a model to build an estimator for it that depends on the physical parameters $\theta$.

.big[$$ p_w(\theta|X) $$]

This model can then be used as a prior for the inference of new data points.


---
class: middle
# Program for the next cycles

.medium[**Cycle 2**]

Introduce .blue[specific architectures] needed to encode heterogeneous data (images, text, linked data)

_Convolution neural networks (CNN) / Transformers (TF) / Graph neural networks (GNN)_

--
.medium[**Cycle 3**]

Learn about generative and multi-modal models, and how they can be used for Bayesian inference in astrophysics 

_GANs, VAE, score-based models, Foundation models, Simulation-based inference (SBI)_
...
https://aboucaud.github.io/slides/2024/euclid-school-ml-cycle1  
</br>
